...
work_dir = 'out/%s_20181223/' % net_name
...
parser.add_argument('--save-dir', type=str, default=saved_dir,
...
parser.add_argument('--logging-file', type=str, default=work_dir + 'train_imagenet.log',
...
def train(ctx):
    ...
    if opt.resume_epoch == 0:
        ...
        if opt.mode == 'hybrid':
            ...
            gstep = 0
            shape = {}
            net.forward(mx.nd.ones((2, 3, opt.input_size, opt.input_size), ctx=ctx[0], dtype=opt.dtype))
        ...
        net.collect_params().load(saved_dir + "/%s-%04d.params" % (net_name, opt.resume_epoch - 1))
    ...
    for epoch in range(opt.resume_epoch, opt.num_epochs):
        ...
        progbar = Progbar(target=max_step, prefix='Train - ', stateful_metrics=[])
        tic = time.time()
        if opt.use_rec:
            train_data.reset()
        train_metric.reset()
        ...
        for i, batch in enumerate(train_data):
            ...
            logsw.add_scalar('loss_batch', {net_name + '_train_loss': batch_loss}, global_step=gstep)
            progbar.update(i + 1)
            ...
            if opt.log_interval and not (i+1)%opt.log_interval:
                ...
                logger.info('\nEpoch[%d] Batch [%d]\tSpeed: %f samples/sec\t%s=%f\tlr=%f'%(
                             epoch + 1, i, batch_size*opt.log_interval/(time.time()-btic),
            ...
            for arg in net.collect_params().values():
                if np.isnan(arg._reduce().asnumpy()).any():
                    ...
                    net.export(saved_dir + '/%s_nan_parameter' % (net_name), epoch=epoch)
                    sys.exit()
        ...
        logger.info('[Epoch %d] training: %s=%f'%(epoch + 1, train_metric_name, train_metric_score))
        logger.info('[Epoch %d] speed: %d samples/sec\ttime cost: %f'%(epoch + 1, throughput, time.time()-tic))
        logger.info('[Epoch %d] validation: err-top1=%f err-top5=%f'%(epoch + 1, err_top1_val, err_top5_val))
        net.export(saved_dir + '/%s' % (net_name), epoch=epoch)
        if err_top1_val < best_val_score:
            best_val_score = err_top1_val
            net.export(saved_dir + '/%s-best-%.4f' % (net_name, best_val_score), epoch=epoch)
