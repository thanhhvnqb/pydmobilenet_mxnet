...
import pydnet64 as pydnet

context = [mx.gpu(1)]
net_name = 'pydnet_0_25'

get_pydnet = getattr(pydnet, net_name)
work_dir = 'out/20190121_%s/' % net_name
...
parser.add_argument(
    ...
    type=int,
    default=64,
    help='size of the input image size. default is 64')
parser.add_argument(
    '--crop-ratio',
    type=float,
    default=0.875,
...
def get_data_loader(data_dir, batch_size, num_workers):
    ...
    train_data = gluon.data.DataLoader(
        ImageNet64(data_dir, train=True).transform_first(transform_train),
        batch_size=batch_size,
        shuffle=True,
        last_batch='discard',
        num_workers=num_workers)
    val_data = gluon.data.DataLoader(
        ImageNet64(data_dir, train=False).transform_first(transform_test),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers)
